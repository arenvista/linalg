...
public:
    ...
    bool  isContiguous() const;  // row-major with no gaps: rowStride == cols, colStride == 1
    bool  isEmpty() const;
    ...
    void copyFrom(const MatrixView& source); // elementwise; source must not alias
...
public:
    ...
    ConstMatrixView();
    ConstMatrixView(const T* data, Index rows, Index cols, Index rowStride, Index colStride);
