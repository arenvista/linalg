...
template <typename T> LinearSolver<T>::LinearSolver() {
    throw LinalgError("not implemented: linalg::LinearSolver<T>::LinearSolver");
}

template <typename T> LinearSolver<T>::LinearSolver(const Options &options) {
    throw LinalgError("not implemented: linalg::LinearSolver<T>::LinearSolver");
}
...
template <typename T>
typename LinearSolver<T>::Method::Kind LinearSolver<T>::selectMethod(
    const Matrix<T> &a
) const {
...
template <typename T>
Vector<T> LinearSolver<T>::refine(
    const Matrix<T> &a,
    const Vector<T> &b,
    const Vector<T> &x,
    Index            steps
...
template <typename T>
void LinearSolver<T>::equilibrate(
    Matrix<T> &a,
    Vector<T> &rowScale,
    Vector<T> &colScale
) const {
...
#define LINALG_INSTANTIATE(SCALAR) template class LinearSolver<SCALAR>;
